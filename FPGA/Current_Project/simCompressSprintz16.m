...
function message_packed = simCompressSprintz16(raw_message)
    ...
    message_zigzag = uint16(zeros(1,size(raw_message,2)));
    
    % y_i = abs(x_i)*2 
    % If x_i < 0  ----> y_i = y_i - 1
    for i = 1:size(raw_message,2)
        message_zigzag(i) = uint16(abs(2*message_delta(i)));
    ...
    mask_arr = uint16(zeros(1,17));
    for i=1:16
        mask_arr(i) = uint16(2^16 - bitshift(1, i-1));
    end

    % initialise packed array, size is unknown
    message_packed = uint8.empty;
    
    % leading_zeros = 16; % assume best case scenario
    % fprintf("Begin packing \n")
    
    % For loop for every 8 bytes
    for i=1:8:size(raw_message,2)
        leading_zeros = 16; % For every new block, reset the leading_zeros
        ...
        for j=0:7
            if leading_zeros > 1 % Stop when leading zeros is 1 or 0 because they are treated the same
                for k=leading_zeros:-1:0 % Only check the worst scenarios for every byte of the block
                    if bitand(message_zigzag(i+j),mask_arr(17-k)) == 0
                        leading_zeros = k;
        ...
        n_pack_size = 16 - leading_zeros; % This number indicates how many bits should be taken from each byte
        ...
        if n_pack_size >=15
            message_packed = [message_packed, uint8(15)];
        else
