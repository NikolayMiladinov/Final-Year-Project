function decompressed_message = decompressSprintz16(compressed_message)
    ...
    for i=1:size(compressed_message,2)
        ...
        if check_width == 1
            ...
            if pack_bits_left == 8 && pack_bits_left<data_bits_left
                
                bit_array = bitget(compressed_message(i),8:-1:1);
                if data_bits_left == pack_byte_width
                    decompressed_message = [decompressed_message uint16(bit2int(bit_array',size(bit_array,2)))];
                else
                    decompressed_message(end) = bitshift(decompressed_message(end),8);
                    decompressed_message(end) = decompressed_message(end) + uint16(bit2int(bit_array',size(bit_array,2)));
                end
                
                pack_bits_left = 0;
                data_bits_left = data_bits_left - 8;
            end
    ...
    decompress_zigzag = int16(zeros(1,size(decompressed_message,2)));

    % Zigzag decode
    for i = 1:size(decompress_zigzag,2)
        if mod(decompressed_message(i),2)==1
            decompress_zigzag(i) = int16((decompressed_message(i) + 1)/2);
            decompress_zigzag(i) = -decompress_zigzag(i);
        else
            decompress_zigzag(i) = int16(decompressed_message(i)/2);
        end
    end

    decompressed_message = int16(decompress_zigzag);
